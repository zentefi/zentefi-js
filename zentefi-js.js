...
Language.parseLanguage = function(s)
{
	if(!s) return Language.language;
	
    var l = s.replace(/[^\w]+/g).toLowerCase();
	
	if(Array.contains(Language.availables, l))
	{
		return l;
	}
...
//
    ...
    jQuery.fn.checked=function(checked){
        if(arguments.length == 0) return Boolean(this.attr('checked'));
    ...
    jQuery.fn.fillWindowHeight = function(fillHeight) {
        ...
        if(!$this.data('__fill_height_fn__')) {
            ...
            var fillHeightFn = function() {
                ...
                $this.css({'min-height': avalHeight - (documentHeight - $this.outerHeight()) + (fillHeight ? fillHeight : 0)});
    ...
    $.fn.loadMethod = function(method, url, data, callback)
    {
        var $this = this;
    ...
    $.fn.loadGet = function(url, data, callback)
    {
        return this.loadMethod('get', url, data, callback);
    };

    $.fn.loadPost = function(url, data, callback)
    {
        return this.loadMethod('post', url, data, callback);
    };
    ...
    jQuery.fn.scrollIntoView = function(bottom)
    {
        if(this.length > 0)
        {
            this[0].scrollIntoView(arguments.length > 0 ? bottom : true);
        }
...
Navigation.__backURL = document.referrer;

Navigation.getBackURL = function()
{
    return Navigation.__backURL;
};

Navigation.goBack = function()
{
    return Navigation.go(Navigation.getBackURL());
};
...
Date._defaultInputFormat = '%Y-%m-%d %H:%H:%S'; 	/* Expresion de parseo por defecto */
...
jQuery.modal.close = function(iter)
{
    ...
    {
        $('.modal-scrollable .in').last().modal('hide');
    }
...
{
    $('.modal-scrollable .in').each(function(index, item){
       $(item).modal('hide');
    });
};
...
{
    return $('.modal-scrollable .in').last();
};
...
jQuery.fn.zmodal = function(options)
{
    ...
    //if($this.find('.modal-dialog').length == 0 && !$this.is('.modal-dialog'))
    //{
    //    if($this.find('.modal-body').length == 0)
    //    {
    //        var modalDialog = $('<div />').addClass('modal-dialog').html($this.html());
    //        var modalBody = $('<div />').addClass('modal-body').appendTo(modalDialog);
    //        $this.html(modalDialog);
    //    }
    //    else
    //    {
    //        var modalDialog = $('<div />').addClass('modal-dialog').html($this.html());
...
{
    var dialog = $('<div />').prependTo('body');
    var modalDialog = $('<div />').addClass('modal-dialog').appendTo(dialog);
    var dialogBody = $('<div />').addClass('modal-body').appendTo(modalDialog).width(300);

    var block = $('<div />').addClass('loading modal-loading').appendTo(dialogBody);

    var contentBlock = $('<div />').addClass('content-block').appendTo(block).append('<br />');
    var iconSpan = $('<span />').addClass('fa fa-circle-o-notch fa-spin icon').appendTo(contentBlock);
    contentBlock.append('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;');
    var textSpan = $('<span />').html(Language.text('loading') + '...').addClass('text').css({'font-weight': 'bold'}).appendTo(contentBlock);
    contentBlock.append('<br /><br />');
...
jQuery.zmodal.alert = function(msg, callback, options)
{
    var dialog = $('<div />').prependTo('body');
    var modalDialog = $('<div />').addClass('modal-dialog alert-dialog').appendTo(dialog);
    var dialogBody = $('<div />').addClass('modal-body').appendTo(modalDialog).append(msg);
    var dialogFooter = $('<div />').addClass('modal-footer').appendTo(modalDialog);

    var button = $('<button type="button" />').addClass('btn btn-primary first-focus').html(Language.text('accept')).appendTo(dialogFooter);
...
jQuery.zmodal.alertClose = function(msg, callback, options)
{
...
jQuery.zmodal.alertCloseAll = function(msg, callback, options)
{
    return jQuery.zmodal.alert(msg, function() {
...
jQuery.zmodal.confirm = function(html, callback, options)
{
    var dialog = $('<div />').prependTo('body');
    var modalDialog = $('<div />').addClass('modal-dialog alert-dialog').appendTo(dialog);
    var dialogBody = $('<div />').addClass('modal-body').appendTo(modalDialog).append(html);
    var dialogFooter = $('<div />').addClass('modal-footer').css({'margin-top': 10}).appendTo(modalDialog);

    var confirmButton = $('<button type="button" />').addClass('btn btn-danger first-focus').html(Language.text('accept')).appendTo(dialogFooter);
    ...
    var cancelButton = $('<button type="button" />').addClass('btn').html(Language.text('cancel')).appendTo(dialogFooter);
				...
				var updateImageSize = function()
				{
					if(imageOptions['fill-window'])
					{
						...
						if(imageWidth > 0 && imageHeight > 0 && avalWidth > 0 && avalHeight > 0)
						{
							...
							dialogBlock.find('.images-list-buttons-block').css({'width': newWidth, 'margin-top': (newHeight/2)});
